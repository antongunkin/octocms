...
vi.mock("./contentStoreFetch", () => ({
  fetchBranchContent: (...args: unknown[]) =>
    mockFetchBranchContent(...(args as [])),
}));
...
function makeBranchStore(
  branch: string,
  overrides?: Partial<BranchStoreData>,
): BranchStoreData {
...
describe("getStoredFile", () => {
  ...
  it("returns null for a missing file", async () => {
    ...
    const result = await getStoredFile(
      "cms/content/post/post-999.json",
      "feat",
    );
...
describe("getStoredFileSha", () => {
  ...
  it("returns null for unknown file", async () => {
    ...
    const sha = await getStoredFileSha(
      "cms/content/post/post-999.json",
      "feat",
    );
    expect(sha).toBeNull();
...
describe("applyMutation", () => {
  it("upserts a new entry", async () => {
    ...
    expect(snapshot!.byCollection.get("post")).toContain(
      "cms/content/post/post-new.json",
    );
  ...
  it("updates an existing entry", async () => {
    ...
    expect(
      snapshot!.byCollection
        .get("post")!
        .filter((p) => p === "cms/content/post/post-1.json"),
    ).toHaveLength(1);
  });
  ...
  it("deletes an entry", async () => {
    ...
    expect(snapshot!.byCollection.get("post")).not.toContain(
      "cms/content/post/post-1.json",
    );
    expect(snapshot!.version).toBe(1);
  });
  ...
  it("deletes a media entry from both indexes", async () => {
    ...
    const snapshot = getStoreSnapshot("feat");
    expect(snapshot!.entries.has("cms/content/media/media-uuid.json")).toBe(
      false,
    );
    expect(
      snapshot!.mediaEntries.has("cms/content/media/media-uuid.json"),
    ).toBe(false);
  });
