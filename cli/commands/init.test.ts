...
describe("initCommand", () => {
  it("creates cms/octocms.config.ts with schema and next.config.ts as thin wrapper in --yes mode", async () => {
    ...
    const octoConfig = readFileSync(
      join(TMP_DIR, "cms", "octocms.config.ts"),
      "utf8",
    );
  ...
  it("creates admin route files", async () => {
    await initCommand(TMP_DIR, { yes: true });
    expect(existsSync(join(TMP_DIR, "src", "app", "cms", "layout.tsx"))).toBe(
      true,
    );
    expect(
      existsSync(join(TMP_DIR, "src", "app", "cms", "[[...path]]", "page.tsx")),
    ).toBe(true);
  });
  ...
  it("updates tsconfig.json with path aliases", async () => {
    ...
    const tsconfig = JSON.parse(
      readFileSync(join(TMP_DIR, "tsconfig.json"), "utf8"),
    );
    expect(tsconfig.compilerOptions.paths["octocms/config"]).toBeUndefined();
    expect(tsconfig.compilerOptions.paths["octocms/*"]).toEqual([
      "./octocms/*",
    ]);
  });
});
